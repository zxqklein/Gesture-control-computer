    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                hand_local = []
                for i in range(21):
                    x = hand_landmarks.landmark[i].x * debug_image.shape[1]
                    y = hand_landmarks.landmark[i].y * debug_image.shape[0]
                    hand_local.append((x, y))
                if hand_local:
                    angle_list = new.hand_angle(hand_local)
                    res_gesture = new.h_gesture(angle_list)  # 一个数组，前面是字符串，后面是id号
                    gesture_str = res_gesture[0]
                    ID1 = res_gesture[1]
                    finger_gesture_history.append(ID1)
                    most_common_id = Counter(finger_gesture_history).most_common()
                    print(gesture_str)
                    cv2.putText(debug_image, gesture_str, (50, 100), 0, 1.3, (0, 0, 255), 2)
                ...
                if time.time() - presstime > 1:
                    ...
                    if detect_mode == 1:
                        if time.time() - presstime_2 > 1:
                            # 静态手勢控制
                            control_keyboard(most_common_id, 666, 'space', keyboard_TF=True, print_TF=True)
                            control_keyboard(most_common_id, 9, 'f', keyboard_TF=True, print_TF=True)
                            control_keyboard(most_common_id, 11, 'like', keyboard_TF=True, print_TF=True)
                            control_keyboard(most_common_id, 6, ']', keyboard_TF=True, print_TF=True)
                            presstime_2 = time.time()

                        # right：右鍵
                        if most_common_id[0][0] == 1 and most_common_id[0][
                            1] == 5:  # most_common_id是一个列表，其中每个元素都是一个元组，元组中包含两个元素，第一个元素是手指姿势的ID，第二个元素是该手指姿势出现的次数
                            if i == 3 and time.time() - presstime_4 > 0.3:
                                ...
                                presstime_4 = time.time()
                        # left：左鍵
                        if most_common_id[0][0] == 8 and most_common_id[0][1] == 5:
                            if i == 3 and time.time() - presstime_4 > 0.3:
