...
def main():
    ...
    # FPS Measurement 读取fps########################################################
    cvFpsCalc = CvFpsCalc(buffer_len=3)

    # Coordinate history 坐标历史记录#################################################################
    history_length = 16
    ...
    # ========= 滑鼠前置作業 =========
    wScr, hScr = pyautogui.size()  # the size of screen
    frameR = 100
    smoothening = 7
    plocX, plocY = 0, 0
    clocX, clocY = 0, 0
    ...
    # speech_0 = gTTS(text="睡眠模式", lang='zh-CN', tld='com')
    # speech_0.save('rest.mp3')
    # speech_0 = gTTS(text="鍵盤模式", lang='zh-CN', tld='com')
    # speech_0.save('keyboard.mp3')
    # speech = gTTS(text="滑鼠模式", lang='zh-CN', tld='com')
    # speech.save('mouse.mp3')
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # print(entrypoint = {keypoint}')
                # print(foremost_common_keypoint_id = {most_common_keypoint_id}')
        ...
        if left_id + right_id > -2:
            if time.time() - presstime > 1:
                ...
                if most_common_ms_id[0][0] == 3 and most_common_ms_id[0][1] == 40:
                    # Gesture six changes to the different mode
                    print('Mode has changed')
